...
export default function Page() {
    ...
    const updateCase = (status: Status, msg: string) => { setCases(cs => cs.map(c => c.id === selected.id ? { ...c, status, updated: 'just now' } : c)); notify(msg) }
    const updateCaseById = (id: string, status: Status, msg: string) => { setCases(cs => cs.map(c => c.id === id ? { ...c, status, updated: 'just now' } : c)); notify(msg) }
    const confirmRisk = () => { setAlerts(as => as.some(a => a.caseId === selected.id) ? as : [{ id: `ALT-00${92 + as.length}`, caseId: selected.id, account: selected.account, amount: selected.amount, risk: selected.risk, status: 'pending', bank: selected.bank }, ...as]); notify('High-risk alert sent to bank fraud desk') }
    if (!role) return <Landing onSelect={setRole} activeCases={cases.length} />
    if (role === 'bank') return <><Topbar role={role} setRole={setRole} /><BankShell alerts={alerts} onAcknowledge={id => { setAlerts(as => as.map(a => a.id === id ? { ...a, status: 'acknowledged', officer: 'R. Mehta' } : a)); notify('Alert acknowledged by Axis Bank') }} onFreeze={(id, officerName) => { const a = alerts.find(x => x.id === id); setAlerts(as => as.map(x => x.id === id ? { ...x, status: 'frozen', officer: officerName } : x)); if (a) setCases(cs => cs.map(c => c.id === a.caseId ? { ...c, status: 'frozen', updated: 'just now' } : c)); notify('Investigator notified — action logged') }} /></>
